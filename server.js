...
function manager(req,res,next){
  if(!['OWNER','MANAGER'].includes(req.user.role)) return res.status(403).json({error:'Manager permission required'});
  next();
}
    ...
    for(const p of cat){
      await client.query(
        `INSERT INTO products(sku,category,name,serving,selling_price,active)
         VALUES($1,$2,$3,$4,$5,$6)
         ON CONFLICT(sku) DO UPDATE SET category=EXCLUDED.category,name=EXCLUDED.name,
         serving=EXCLUDED.serving,selling_price=EXCLUDED.selling_price,active=EXCLUDED.active`,
...
app.get('/api/catalogue', async (req,res)=>{
  const {rows}=await pool.query(`SELECT p.sku,p.category,p.name,p.serving,p.selling_price::float AS selling_price,
    ...
    FROM products p JOIN inventory i USING(sku) WHERE p.active=TRUE ORDER BY p.category,p.name`);
  res.json(rows);
});
